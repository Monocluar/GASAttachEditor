...
public:
	...
	GASATTACHEDITOR_API virtual ~SAddNewGameplayTagWidget();

	GASATTACHEDITOR_API virtual void Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime) override;

	GASATTACHEDITOR_API void Construct(const FArguments& InArgs);
